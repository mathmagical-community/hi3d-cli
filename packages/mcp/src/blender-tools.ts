...
export const BLENDER_TOOLS: ToolDef[] = [
  ...
  recipe('blender_transform', 'Transform', 'Translate (m), rotate (degrees) and/or scale objects, then apply.', { object: OBJECT, translate: z.array(z.number()).length(3).optional(), rotate_deg: z.array(z.number()).length(3).optional(), scale: z.union([z.number(), z.array(z.number()).length(3)]).optional(), apply: z.boolean().optional() }, 'transform'),
  ...
  recipe('blender_hollow', 'Hollow (experimental)', 'Shell the model inward with the given wall thickness to save print material. Experimental: check the result with blender_inspect / blender_repair.', { wall_thickness_mm: z.number().positive().optional(), object: OBJECT }, 'hollow', { destructiveHint: true }),
  {
    name: 'blender_setup',
