...
分支：日常提交到 `develop`，`main` 只放已发布代码。发布：在 `main` 的提交上打 `v*` tag——工作流会拒绝不在 `main` 上的
tag，然后跑三系统 × Node 18 / 20 / 22 冒烟并带 provenance 发布到 npm。参与贡献见 [CONTRIBUTING.md](CONTRIBUTING.md)。
