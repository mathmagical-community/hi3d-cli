...
echo "## transform on a glTF-imported (quaternion-mode) object, hollow safety"
$H --workspace "$WS" blender_run_script 'bpy.ops.wm.read_factory_settings(use_empty=True); bpy.ops.mesh.primitive_cube_add(size=1); o=C.object; o.scale=(0.2,0.1,0.05); bpy.ops.object.transform_apply(scale=True)' 2>/dev/null | j "assert d['ok']"
$H --workspace "$WS" blender_export gen/box.glb 2>/dev/null | j "assert d['ok']"
$H --workspace "$WS" blender_load gen/box.glb 2>/dev/null | j "assert d['ok']"
$H --workspace "$WS" blender_run_script 'result={"mode": C.scene.objects[0].rotation_mode}' 2>/dev/null | j "print('imported rotation_mode:', d['body']['result']['mode'])"
$H --workspace "$WS" blender_transform --rotate-deg 0 0 90 2>/dev/null | j "b=d['body']['before']['dimensions_m']; a=d['body']['after']['dimensions_m']; assert abs(a[0]-b[1])<1e-4 and abs(a[1]-b[0])<1e-4, (b,a); print('rotate 90° about Z swapped X/Y:', b, '->', a, d['body'].get('rotation_mode_switched'))"
$H --workspace "$WS" blender_transform --scale 2 2>/dev/null | j "b=d['body']['before']['dimensions_m']; a=d['body']['after']['dimensions_m']; assert all(abs(x*2-y)<1e-4 for x,y in zip(b,a)), (b,a); print('scale 2 via shell ok')"
$H --workspace "$WS" blender_transform --translate 0 0 0.05 2>/dev/null | j "assert d['ok']; print('translate ok')"
$H --workspace "$WS" blender_load gen/cube.glb 2>/dev/null | j "assert d['ok']"
$H --workspace "$WS" blender_hollow --wall-thickness-mm 2 2>/dev/null | j "b=d['body']['before']; a=d['body']['after']; assert not d['body']['rolled_back'], d['body']; assert all(abs(x-y)/x<0.01 for x,y in zip(b['dimensions_m'],a['dimensions_m'])), (b,a); assert a['faces']==2*b['faces'], (b['faces'],a['faces']); print('hollow kept size', a['dimensions_m'], 'faces', b['faces'],'->',a['faces'])"
$H --workspace "$WS" blender_run_script 'bpy.ops.wm.read_factory_settings(use_empty=True); bpy.ops.mesh.primitive_monkey_add(size=0.12); bpy.ops.object.modifier_add(type="SUBSURF"); C.object.modifiers[0].levels=2; bpy.ops.object.modifier_apply(modifier=C.object.modifiers[0].name)' 2>/dev/null | j "assert d['ok']"
$H --workspace "$WS" blender_hollow --wall-thickness-mm 2 2>/dev/null | j "b=d['body']['before']['dimensions_m']; a=d['body']['after']['dimensions_m']; assert max(a)/max(b) < 1.02, (b,a); print('organic hollow: size kept', [round(x,4) for x in a], 'rolled_back=', bool(d['body']['rolled_back']))"
$H --workspace "$WS" blender_hollow --wall-thickness-mm 2 --even-offset 2>/dev/null | j "b=d['body']['before']; a=d['body']['after']; assert d['body']['rolled_back'] and a['dimensions_m']==b['dimensions_m'] and a['faces']==b['faces'], d['body']; print('blow-up guard: rolled back, mesh unchanged; would have been', d['body']['rolled_back'][0]['dimensions_would_be_m'])"
